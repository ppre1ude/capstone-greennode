...
- AI가 이미지 안의 식재료 후보와 나눔 가능 여부를 판단한다.
...
| ---------------- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------- |
| 농산물 등록 흐름 | 카메라, 갤러리 | AI 분석 결과를 보여주고, 나눔 기준 통과 항목만 공유 냉장고 선택과 보관 QR 인증으로 보낸다. 여러 품목은 QR 화면에서 순차 인증한다.                     | `POST /posts` 성공 후 `status=pending_store`, 보관 QR 인증 대기 | QR 없는 별도 생성 flow                      |
...
- 사진 한 장에서 식재료 후보와 내부 신선도 등급을 분석한다.
...
- 공급자는 AI 분석 결과와 제외 품목을 확인하고 공유 냉장고를 선택한다.
- 권장 수령일/보관 기한은 프론트가 수동 입력받지 않고 서버가 품목/신선도 보관 규칙에 따라 자동 산정한다. `expirationDate`는 선택 필드이며, 앱은 기본적으로 `null`을 보내 서버 산정값을 사용한다.
- 등록이 완료되면 근처 사용자에게 푸시 알림을 보낸다.
- QR 생명주기 반영 후 등록은 `pending_store`로 시작하고, 보관 QR 인증이 성공해야 수요자-facing 목록에 노출된다.
- 서버는 전체 차단 대상 이미지에는 `imageToken`을 발급하지 않고, 무효/만료 토큰으로는 최종 등록을 막는다. 프론트의 나눔 가능 가드는 사용자 경험용이며 서버가 최종 방어선이다. 등록 payload는 `imageToken`, `fridgeId`, `flow`, 선택 `expirationDate`이고 AI 메타데이터, `selectedDetectionId`, `bbox`는 재전송하지 않는다. `shareable=false` 감지 품목은 제외 대상으로 안내한다.
