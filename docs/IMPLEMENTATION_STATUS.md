...
8. 완료, VM/API QA 및 emulator UI QA 통과: 냉장고별 나눔 식재료 조회 API 연동. 지도에서 선택한 냉장고의 available 목록을 표시하고 항목 탭 시 상세로 이동한다. 단, Post AI 메타데이터 null 때문에 표시명/상태 fallback 재검증은 P0 백엔드 수정 후 필요하다.
