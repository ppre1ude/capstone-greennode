...
**보관 배치**:
한 번의 일괄 등록으로 생긴 물리적 보관 묶음. 앱은 여러 `pending_store` 품목을 QR 화면에서 순차 인증할 수 있지만, 사용자-facing 신청 단위는 계속 **나눔 식재료**다.
...
- AI 분석은 한 이미지에서 하나 이상의 **식재료** 후보를 만들 수 있다. 각 후보는 **신선도 등급**, **나눔 가능 여부**, 선택적 `bbox`를 가진다.
...
- 현재 **권장 수령일/보관 기한**은 프론트가 직접 계산하거나 입력받지 않고, 최종 등록 시 서버가 `imageToken`에 연결된 품목/신선도 정보를 기준으로 자동 산정한다. 프론트가 `expirationDate`를 보내지 않거나 `null`로 보내면 서버 산정값이 source of truth다.
