...
**QR 인증**:
사용자가 냉장고 QR을 스캔하고, 서버가 로그인 사용자, 진행 중인 행동, 공유 냉장고, 30분 제한을 검증해 보관 확인 또는 수령 확인을 처리하는 행위.
...
- MVP에서는 `requested` 이후 공급자 승인 행동을 요구하지 않는다. 운영 문제는 초기에는 수동 운영으로 제어하고, **냉장고 운영자** 화면은 후순위 제품 범위로 둔다.
- **냉장고 운영자**는 시스템 계정/권한 전체를 관리하는 admin이 아니라, 특정 공유 냉장고의 현장 상태를 점검하는 운영 역할이다.
- Post-MVP QR 흐름에서는 `requested`를 30분 **임시 선점**으로 확장한다. 현재 MVP 구현은 여전히 신청 접수 의미로 유지한다.
- QR 흐름에서 **등록 대기** 나눔 식재료는 **QR 인증** 전까지 홈/지도/냉장고 available 목록에 노출하지 않는다.
- **냉장고 QR**은 공유 냉장고 식별자일 뿐이고, 실제 인증은 서버가 사용자, pending action, fridgeId, 30분 제한을 검증해 처리한다.
...
| `pending_store` | QR 보관 인증 전 등록 대기 | Post-MVP QR/inventory PRD 대상                 |
| `requested` | MVP에서는 신청 접수, QR 도입 후에는 30분 임시 선점 | 백엔드 구현됨, QR 의미 확장은 후속             |
| `reserved`  | 특정 수요자에게 예약 확정됨 | 후속                                           |
| `completed` / `picked_up` | 실제 수령 완료 또는 수령 확인 이벤트 | QR 수령 확인 도입 후 확정                      |
| `cancelled` | 등록자/냉장고 운영자가 취소 | 후속 또는 수동 운영                            |
| `expired`   | 나눔 가능 기간이 지남       | 후속                                           |
| `disposed`  | 냉장고 운영자가 폐기/회수 완료 | Post-MVP 운영자 흐름 대상                      |
...
| 보관 QR 인증     | 공급자        | `pending_store`, 선택한 공유 냉장고, 30분 안의 냉장고 QR 스캔 | `pending_store -> available`    | QR 도입 후에는 실제 보관 확인 전까지 public 목록에 노출하지 않는다.                                  |
| 수령 QR 인증     | 수요자        | `requested`, 30분 안의 냉장고 QR 스캔                           | `requested -> completed/picked_up` | QR 도입 후 `requested`는 임시 선점이며, 제한 시간이 지나면 available로 복원한다.                    |
| 수령 완료        | 수요자/냉장고 운영자 | 후속 버전의 예약/수령 검증 수단 필요                       | `completed`                     | QR, 비밀번호 토큰, 냉장고 운영자 확인을 설계한 뒤에만 제품 흐름에 넣는다.                           |
| 냉장고 운영자 조정 | 냉장고 운영자 | 냉장고 운영자 화면 또는 수동 운영 절차 필요                       | `cancelled`, `expired` 등       | 냉장고 운영자 화면은 제품 범위에는 있지만 MVP 구현 범위 밖이다.                                     |
...
- 냉장고 운영자 화면은 제품 범위에는 포함되지만 MVP 구현 범위에서는 제외한다. 초기 운영 제어는 수동 운영으로 처리한다.
