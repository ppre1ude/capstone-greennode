...
- **나눔 신고**는 운영자 검토 큐에 들어가는 사건이다. 태그가 아니라 단일 `reasonId`와 `status`를 가진다.
- 신고 상태는 `open`, `reviewing`, `resolved`, `dismissed`를 사용한다.
- 공급자 신뢰 뱃지는 별점이나 점수가 아니라 QR 생명주기, 좋은 평가, 열려 있는 신고 검토 상태를 요약한다.
...
| `src/features/trust/report.ts` | 신고 사유, 신고 처리 상태, 열린 신고 판정만 관리 |
...
- 열린 신고 집계는 `open`과 `reviewing`만 포함한다.
- 신고가 공급자 공개 점수처럼 즉시 노출되는 구조를 만들지 않는다.
...
| 공급자 신뢰 뱃지 | 표시용 칩 | 신뢰 신호 요약, 입력 아님 |
...
- `__tests__/shareFeedback.screen.test.tsx`: 신고 사유가 radio option으로 렌더링되고 단일 `reasonId`와 `open` 상태로 저장되는지 검증
- `__tests__/trustFeedback.policy.test.ts`: 평가 태그와 신고 사유가 분리된 도메인 카탈로그인지, 열린 신고 상태 판정이 맞는지 검증
