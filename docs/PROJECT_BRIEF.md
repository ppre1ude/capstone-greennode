...
- 카메라 셔터 촬영은 에뮬레이터에서 실패했고, 갤러리 선택 경로만 실제 AI 파이프라인까지 검증됐다.
- AI 응답은 현재 단일 대표 객체(`detectedFruit`) 중심으로 처리한다. multi-object detection은 다음 스프린트 연구/계약 설계 항목이다.
- `Stale/Bad/Rotten` 또는 `canShare=false` 상태에서 실제 등록을 차단하는 guard가 아직 없다.
...
- **남은 작업**: 부패 의심 상태와 낮은 confidence에서 실제 등록을 막거나 확인 필요 상태로 분기해야 한다.
