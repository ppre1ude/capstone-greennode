...
| 도메인/용어 정렬 | 7 | 문서는 강함. UI 일부는 `AI 신뢰도`, `기한 만료`, `푸시 전송`처럼 과신/확정처럼 읽힐 위험 있음 |
...
- `AI 신뢰도 100%`: 품질 보증처럼 들린다. 문서상 confidence는 softmax max 확률이며 차단 기준이 아니다. `AI 참고 신호`, `분석 확신도` 등으로 완화 권장.
