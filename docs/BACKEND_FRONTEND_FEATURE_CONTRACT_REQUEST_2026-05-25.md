...
2026-05-26 CEO/product review 기준으로 체류시간 자체를 늘리는 기능보다 진행 중인 나눔 lifecycle을 사용자가 추적하고 끝낼 수 있는 계약을 우선한다.

1. 내 나눔/받은 나눔 목록: QR 보관/수령, 신청 상태 확인, 일반 사용자 운영 흐름을 만들기 위한 핵심 화면이다.
2. 알림 서버 목록/읽음: 현재는 로컬 읽음만 가능하므로 기기 변경/재설치 시 이력이 사라진다.
3. 일반 나눔 lifecycle: `requested` 이후 취소, 만료, 완료, QR 수령 실패, 재노출 정책을 확정해야 한다.
4. `/auth/me` 운영자 metadata: 이미 operator inventory가 동작하므로 작은 계약으로 앱 진입 정책을 닫을 수 있다.
5. 프로필 수정/관심 식재료: 제품 완성도는 높이지만 핵심 transaction보다는 후순위다.
6. 검색/통계/소셜/이메일 인증/AI enum 확장: Post-MVP로 별도 일정화한다.
