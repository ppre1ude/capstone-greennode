...
- `Fresh`와 `Normal` 계열 **신선도 등급**은 사용자 흐름에서 모두 `상태가 좋아 보여요`와 `나눔 가능`으로 통합 표시한다.
- `Stale`, `Bad`, `Rotten` 계열은 AI/백엔드가 `Stale`의 의미를 더 정확히 정의하기 전까지 **나눔 기준 미충족**으로 보고 등록하지 않는다.
- `not_food`, `non_food`, `low_quality`, `screenshot`, `ui_screenshot` 계열은 **나눔 기준 미충족** 또는 식재료 사진 확인 실패로 보고 등록하지 않는다.
- `confidenceScore`는 현재 차단 정책이 아니라 보조 표시/검토 신호다. 이 값이 객체 탐지, 신선도 분류, 식재료 여부 중 무엇의 confidence인지 AI/백엔드 계약에서 추가 정의해야 한다.
...
| `requested` | 수요자의 나눔 신청이 접수됨 | MVP 목표 흐름 |
| `reserved` | 특정 수요자에게 예약 확정됨 | 후속 |
| `completed` | 실제 수령 완료 | 후속 |
...
| 나눔 신청 | 수요자 | 나눔 식재료가 `available`, 신청자가 작성자가 아님 | `available -> requested` | 현재 제품 가정은 첫 신청 접수 후 추가 신청 CTA를 막는 것이다. |
...
- `Fresh`와 `Normal`의 세부 차이는 내부 데이터/추천/QA에서만 쓰고, 사용자 흐름에서는 같은 나눔 가능 상태로 다룬다.
...
>
> **Dev:** "AI가 Fresh와 Normal을 다르게 주면 사용자에게 둘 다 보여줘야 하나요?"
> **Domain expert:** "주요 흐름에서는 합칩니다. 둘 다 `상태가 좋아 보여요`와 `나눔 가능`으로 보여주고, 내부 정책과 QA에서만 구분하세요."
>
> **Dev:** "confidence가 45%면 등록을 막나요?"
> **Domain expert:** "아직은 아닙니다. `confidenceScore`가 무엇의 confidence인지 불명확하므로 보조 표시/검토 신호로만 씁니다."
>
...
- `Stale`의 의미가 아직 AI/백엔드와 충분히 합의되지 않았다. 해결 전까지는 보수적으로 **나눔 기준 미충족**으로 분류한다.
- `confidenceScore`의 의미가 불명확하다. 해결 전까지 등록 차단 기준으로 쓰지 않고 보조 표시/검토 신호로만 쓴다.
- 첫 신청 이후 추가 신청을 막을지에 대해서는 기획 확인이 남아 있다. 현재 권장 정책은 첫 신청 접수 후 `requested`로 전환하고 추가 CTA를 비활성화하는 것이다.
