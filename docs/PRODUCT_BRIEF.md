...
- FCM은 토큰 등록과 빈 알림함 탭까지만 앱에서 구현됐고, foreground/background 수신 처리와 읽음 상태는 미구현이다.
- 채팅은 MVP에서 보류하고 알림함으로 축소했다. 유저 통계와 탄소 절감액은 실제 지표 API가 없어 `준비 중`으로 표시한다. 소셜 로그인, 이메일 verification은 미구현 상태다.
...
- **현재 UI 범위**: 빈 알림함 탭
- **미구현**: 실제 알림 수신 handler, 알림 목록 API, 읽음 상태, 알림 탭 데이터 연동
