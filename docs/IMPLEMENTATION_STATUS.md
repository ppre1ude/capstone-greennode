...
- 남은 작업:
  - 소셜/관심 기능은 MVP 범위 밖으로 유지한다.
  - 서버 저장형 알림 읽음 상태와 실제 활동 지표 API는 앱 client/계약/로컬 fallback까지 연결됐고, 최신 live VM/OpenAPI 재검증만 남았다.
  - 프로필 수정, 내 나눔, 받은 나눔, 알림함 메뉴 연결은 현재 코드 기준 완료됐다.
