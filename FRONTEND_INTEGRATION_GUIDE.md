...
|------|------|------|------|
| image | File | ✅ | 촬영한 사진 |
| food_name | string | ✅ | 식재료 이름 (예: "사과") |
| user_hint | string | ❌ | 추가 설명 (예: "어제 샀어요") |

> ⚠️ Form 필드명은 **snake_case** (`food_name`, `user_hint`)
...
// ⚠️ data 필드에 JSON "문자열" (imageToken 포함!)
...
### 홈 화면
```
상단: GET /fridges/nearby → 근처 냉장고 카드
하단: GET /posts/nearby → 근처 게시글 슬라이드
```
...
- [ ] **게시글 등록 시 이미지 파일 보내지 않음** → imageToken만 JSON에 포함
- [ ] generate에서 400 수신 시 `message`를 Toast/Alert로 표시
- [ ] imageToken은 1시간 내 사용 (만료 시 다시 촬영)
- [ ] generate API Form 필드는 `snake_case` (`user_hint`)
