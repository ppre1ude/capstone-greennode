...
> **Domain expert:** "아니요. `confidenceScore`는 신선도 분류 모델이 Fresh/Mid/Stale 중 하나로 분류한 softmax max 확률입니다. 값만으로 막지 않고 보조 표시/검토 신호로 씁니다."
>
...
- confidence 표시 threshold는 아직 프론트 UX 결정이 남아 있다. 현재 앱은 60% 미만을 확인 필요로 보지만, 백엔드 활용 가이드는 0.9 미만을 확인 필요 구간으로 본다.
