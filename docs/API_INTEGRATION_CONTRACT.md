...
> false-positive 계약: 비식재료/스크린샷/저품질 이미지는 `Fresh`로 통과시키지 않는다. 서버는 generate 400의 `message/detail` 또는 200 응답의 `aiAnalysis.rejectionReason`/`aiAnalysis.reviewReason`에 `not_food`, `non_food`, `low_quality`, `screenshot`, `ui_screenshot`, `multi_object_review`, `review_required` 중 하나를 담는다. 앱은 등록 불가 사유는 차단하고, 검토 사유는 `확인 필요`로 표시한다.
