...
const FORCE_STATIC_SCAN_SURFACE = false;
const FORCE_DEMO_AI_ANALYSIS = false;

const buildDemoGenerateResult = (): GenerateResult => ({
  ...
  imageToken: `demo-image-token-${Date.now()}`,
});
...
const CameraScanScreen = ({ navigation }: Props) => {
  ...
  useEffect(() => {
    if (!FORCE_STATIC_SCAN_SURFACE && !hasPermission) {
      requestPermission();
    }
  }, [hasPermission, requestPermission]);

  useEffect(() => {
    if (!FORCE_STATIC_SCAN_SURFACE && hasPermission && device) {
      Animated.loop(
        Animated.sequence([
  ...
  const processImage = async (
    ...
    setIsAnalyzing(true);
    try {
      if (FORCE_DEMO_AI_ANALYSIS) {
        await new Promise(resolve => setTimeout(resolve, 900));
        navigation.replace('AnalysisResult', {
          result: buildDemoGenerateResult(),
          imageUri: uri,
        });
        return;
      }
  ...
  if (FORCE_STATIC_SCAN_SURFACE) {
    return renderCameraFallback(
      'AI 신선도 스캔',
      '사진 한 장으로 나눔 가능 여부를 확인합니다.',
      <DSButton
        label="갤러리에서 선택하기"
        size="medium"
        style={styles.galleryFallbackButton}
        textStyle={styles.galleryFallbackButtonText}
