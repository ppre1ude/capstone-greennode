...
- 원래 MVP 흐름은 `등록 -> 발견 -> 신청 접수`이며, 상태 전환 기준은 `available -> requested`다. 이 기준은 Android/API 검증으로 이미 닫혔다.
- 이번 주 작업 범위는 MVP가 아니라 MVP 이후 확장 기능이다. `냉장고 QR 인증`, `진행 중인 나눔`, `내 나눔/받은 나눔`, `운영자 콘솔 진입`, `알림/나눔 생명주기 확장`을 API/화면 연결 기준으로 검증했다.
